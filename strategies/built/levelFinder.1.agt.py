...
How it works:
1. This strategy identifies "Order Blocks" (OBs) using our optimized and repaired
   pure technical OB detection module in `ta/patterns/ob.py`.
2. Entry triggers on the close of the candle that solidifies (confirms) the order block.
3. Stop Loss (SL): Placed 1 tick below the order block (if bullish) or 1 tick above the order block (if bearish).
4. Take Profit (TP) splits:
   - TP1 at 1:1 + fees and slippage (33% of position size)
   - TP2 at 1:2 + fees and slippage (33% of position size). On TP2 fill, the SL moves to break-even!
   - TP3 at 1:3 + fees and slippage (34% of position size).
5. All TP prices are mathematically fee-and-slippage aware, utilizing the core
   calculators in `tools/trading_utils.py`.
"""
...
class LevelFinderStrategy(JBaseStrategy):
    def __init__(self, config_overrides: Optional[Dict] = None, simulator=None):
        ...
        # Warm-up history requirement
        self.required_history = {self.params["execution_tf"]: self.params["ob_period"] + 100}
    ...
    def get_entry_signal(self, market_data: Dict) -> Optional[Dict]:
        ...
        ohlcv = self._get_ohlcv(symbol, exec_tf)
        if not ohlcv or len(ohlcv) < self.required_history[exec_tf]:
            return None
        ...
        self.logger.info(f"[{symbol}] {target_ob['type'].upper()} OB Solidified at index {target_ob['index']}!")
