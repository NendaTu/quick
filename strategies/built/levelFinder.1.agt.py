...
## Overview
This strategy identifies "Order Blocks" (OBs) using our optimized and repaired
pure technical OB detection module in `ta/patterns/ob.py`. It confirms OBs strictly
on closed candles, placing limit orders exactly at the close of the solidification (impulse) candle,
aiming to capture high-probability institutional price reversals.
...
## The Intended Flow
1. **Discovery**: Call `detect_order_blocks()` on the closed candles history.
2. **Solidification Check**: Verify if a fresh Bearish or Bullish OB has just solidified (formed) at `ohlcv[-2]`.
