...
def setup_console_tee(console_log_path=None):
    ...
    if console_log_path is None:
        ...
        console_log_path = f"docs/temp/{start_dt}.console-log.txt"
...
def setup_signals_logging(start_time: float, symbol: str, side: str, strategy_id: str, scoring_result: dict, log_dir="docs/temp"):
    """
    Centralized writer for the signals-log.txt file. Logs ALL proposed signals (whether entered or rejected).
    """
    os.makedirs(log_dir, exist_ok=True)
    start_dt = datetime.fromtimestamp(start_time).strftime("%Y%m%d_%H%M%S")
    filepath = os.path.join(log_dir, f"{start_dt}.signals-log.txt")
