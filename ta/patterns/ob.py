...
def detect_order_blocks(ohlcv: List[dict], period: int = 14, impulse_mult: Optional[float] = None) -> Dict:
    """
    ...
    NOTE: 'nearest_ob_type' refers to the most recently formed active OB, not price distance.
    """
    ...
    for i in range(1, len(closed_ohlcv) - 1):
        ...
        # Doji Hardening: treat open == close (within 1e-9 epsilon) as bearish/bullish based on next direction
        is_doji = abs(curr['c'] - curr['o']) < 1e-9
